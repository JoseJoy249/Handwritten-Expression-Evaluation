...
def load_models(path, ver = 1):
    ...
    if ver == 2:
        ...
        json_file1 = open(path+'cnn_v2.json', 'r')
        loaded_model_json1 = json_file1.read()
        json_file1.close()
        CNN_ver2 = model_from_json(loaded_model_json1)
        # load weights into new model
        CNN_ver2.load_weights(path+"cnn_v2.h5")
        adam1 = keras.optimizers.Adam(lr=0.001, beta_1=0.9, beta_2=0.999)
        CNN_ver2.compile(loss=keras.losses.categorical_crossentropy, optimizer=adam1,metrics=['accuracy'] )

        print("Loaded models from disk")
        return adastage1_ver2,adadigits_ver2, adachars_ver2, rfmodel_ver2, MLP_single_ver2, CNN_ver2
